# Legacy post-build code-signing helper for artifacts without Tauri updater signatures.
# Public releases import the certificate before `tauri build` so Authenticode is
# applied before Tauri calculates the updater signature.
#
# Env (CI secrets):
...
$signedUpdaterArtifacts = @($files | Where-Object {
  $_.Extension -eq ".exe" -and (Test-Path "$($_.FullName).sig")
})
if ($signedUpdaterArtifacts.Count -gt 0) {
  throw (
    "Post-build Authenticode signing would invalidate existing Tauri updater " +
    "signatures. Import the PFX before `tauri build` with " +
    "scripts/peeknook-import-windows-certificate.ps1 instead."
  )
}
