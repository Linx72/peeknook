...
export default function Settings() {
  ...
  async function saveE2eSync(e2e: boolean) {
    await updatePeeknookSettings({ e2e_sync: e2e })
    setMsg(e2e ? s.e2eSyncOn : s.e2eSyncOff)
    reload()
  }

  async function saveInterval(sec: number) {
    const v = Math.min(86400, Math.max(60, sec))
    await updatePeeknookSettings({ auto_sync_interval_sec: v })
    setMsg(fmt(s.intervalSaved, { sec: v }))
    reload()
  }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-6 space-y-4 text-sm">
        ...
        <div className="flex items-center gap-2">
          <span className="text-stone-500 text-xs">{s.intervalLabel}</span>
          <input
            type="number"
            min={60}
            max={86400}
            className="w-24 rounded border border-stone-300 px-2 py-1 text-sm"
            value={settings?.auto_sync_interval_sec ?? 300}
            onBlur={(e) => saveInterval(Number(e.target.value))}
            onChange={(e) =>
              setSettings((prev) =>
                prev ? { ...prev, auto_sync_interval_sec: Number(e.target.value) } : prev,
              )
            }
          />
          <span className="text-xs text-stone-400">{s.intervalUnit}</span>
        </div>
        <label className="flex items-center gap-2">
          <input
            type="checkbox"
            checked={settings?.e2e_sync ?? false}
            onChange={(e) => saveE2eSync(e.target.checked)}
          />
          {s.e2eSync}
        </label>
