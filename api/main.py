...
CORS_ALLOWED_ORIGINS = (
    _parse_cors_origins(_cors_origins_raw)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        from open_notebook.peeknook.bootstrap import bootstrap_peeknook
        from open_notebook.peeknook.sync_store import init_sync_store, record_event

        init_sync_store()
        await bootstrap_peeknook()
        record_event("system", "peeknook", "startup", {"version": "0.1.0"})
        from open_notebook.peeknook.auto_sync import start_auto_sync

        start_auto_sync()
    except Exception as e:
        logger.warning(f"PeekNook bootstrap encountered errors: {e}")
    ...
    try:
        from open_notebook.peeknook.auto_sync import stop_auto_sync

        stop_auto_sync()
    except Exception:
        pass
    try:
        from open_notebook.peeknook.standalone import stop_embedded_services

        stop_embedded_services()
    except Exception as exc:
        logger.warning(f"Embedded service shutdown encountered errors: {exc}")
    logger.info("API shutdown complete")


app = FastAPI(
    title="PeekNook API",
    description="API for PeekNook — peek into your research sources",
    lifespan=lifespan,
)

if _cors_origins_raw is None:
    logger.warning(
        "CORS_ORIGINS is not set — API accepts only the built-in localhost and "
        "Tauri origins. Set CORS_ORIGINS explicitly for another frontend origin."
    )
else:
...
app.include_router(peeknook.router, prefix="/api", tags=["peeknook"])
app.include_router(languages.router, prefix="/api", tags=["languages"])


@app.get("/")
async def root():
    return {"message": "PeekNook API is running"}
