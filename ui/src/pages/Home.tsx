...
import { getCloudHealth, getSetupStatus, type CloudHealth, type SetupStatus } from '../api'
import ShipStatusBanner from '../components/ShipStatusBanner'
...
export default function Home() {
    ...
    getSetupStatus()
      .then((st) => {
        ...
        if (st.cloud_url) {
          getCloudHealth()
            .then(setCloudHealth)
            .catch(() => setCloudHealth({ configured: true, ok: false }))
        }
      })
  ...
  return (
    <div className="space-y-6">
      ...
      {cloudHealth?.configured && cloudHealth.url && (
        <section
        ...
        >
          <p>
            {cloudHealth.ok
              ? fmt(h.cloudHealthOk, { version: cloudHealth.version ?? '?' })
              : h.cloudHealthFail}
          </p>
          <a
            className="mt-2 inline-block text-xs underline"
            href={`${cloudHealth.url.replace(/\/$/, '')}/health`}
            target="_blank"
            rel="noreferrer"
          >
            {h.cloudHealthLink}
