...
class TestParseSourceFormData:
    """Multipart form may send both notebook_id and notebooks (Vite UI does)."""

    def test_both_notebook_fields_prefers_notebooks_list(self):
        from api.routers.sources import parse_source_form_data

        source_data, _file = parse_source_form_data(
            type="upload",
        ...
        assert source_data.notebooks == ["notebook:abc"]
        assert source_data.notebook_id is None
