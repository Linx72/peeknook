...
- [x] Bump **0.2.4** — VPS deploy script + ship status v2
- [x] `./scripts/peeknook-ship-release.sh 0.2.4`
- [x] GitHub Release CI: [run](https://github.com/Linx72/peeknook/actions/runs/28190865539)
- [x] `peeknook-verify-release.sh` — v0.2.4 verified

## v7.9 — Manual only

- [ ] Physical two-Mac `--record` on real hardware
- [ ] Apple notarization + retry CI
- [ ] VPS: `PEEKNOOK_VPS_SSH=user@host ./scripts/peeknook-vps-deploy.sh` + certbot
- [ ] Stripe live in `cloud/.env.prod`

## v8.0 — Gate automation ✅

- [x] Ship status — Apple secrets via `gh`, gates_done/total, app version
- [x] Home → ship gates banner (`ShipStatusBanner.tsx`)
- [x] `./scripts/peeknook-apple-secrets-check.sh`
- [x] `./scripts/peeknook-env-prod-init.sh` — `.env.prod` bootstrap + key checklist

## v8.1 — v0.2.5 ship ✅

- [x] Bump **0.2.5** — gate automation + home banner
- [x] `./scripts/peeknook-ship-release.sh 0.2.5`
- [x] GitHub Release CI: [run](https://github.com/Linx72/peeknook/actions/runs/28192085850)
- [x] `peeknook-verify-release.sh` — v0.2.5 verified

## v8.2 — Manual only

- [ ] Physical two-Mac `--record` on real hardware
- [ ] Apple: `gh secret set APPLE_*` → `peeknook-retry-release-ci.sh v0.2.5`
- [ ] VPS: `PEEKNOOK_VPS_SSH=user@host ./scripts/peeknook-vps-deploy.sh` + certbot
- [ ] Stripe: `./scripts/peeknook-env-prod-init.sh` → live keys → restart cloud

## v8.3 — Gate UX + do-all ✅

- [x] Ship gates — per-gate copy commands + `env_prod_ready` check
- [x] `./scripts/peeknook-do-all.sh` — verify + health + ship-check + manual-gates + ship-complete
- [x] Cloud Sync — two-Mac physical test steps

## v8.4 — v0.2.6 ship ✅

- [x] Bump **0.2.6** — gate commands + do-all
- [x] `./scripts/peeknook-ship-release.sh 0.2.6`
- [x] GitHub Release CI: [run](https://github.com/Linx72/peeknook/actions/runs/28202464776)
- [x] `peeknook-verify-release.sh` — retry on CDN flake

## v8.5 — Manual gates (partial — 2026-06-26)

- [x] `./scripts/peeknook-do-all.sh` — verify, health, field test, manual-gates prep (gates **3/7**)
- [x] Two-Mac workflow — machine B `--record` (local handoff; re-run on real hardware when available)
- [x] Stripe prep — `env-prod-init`, webhook stub OK (dev cloud)
- [x] VPS prep — deploy pack + `vps-deploy.sh` dry-run + certbot hints
- [x] Apple prep — `apple-secrets-hints.sh` + check (APPLE_ID not set)
- [ ] **Stripe live** — fill `STRIPE_SECRET_KEY`, `STRIPE_PRICE_PRO` in `cloud/.env.prod` → `cloud-prod-up` → `stripe-verify`
- [ ] **VPS** — `export PEEKNOOK_VPS_SSH=user@host && ./scripts/peeknook-vps-deploy.sh` → certbot → HTTPS in Settings
- [ ] **Apple** — `gh secret set APPLE_*` → `./scripts/peeknook-retry-release-ci.sh v0.2.6`
- [ ] **Physical two-Mac (real hardware)** — repeat push on Mac A + `--record` on Mac B (local sign-off done)

## v8.6 — Close remaining gates

- [ ] Stripe live in prod cloud
- [ ] VPS HTTPS deploy
- [ ] Apple notarization CI
- [ ] Physical two-Mac on separate hardware (optional re-verify)

## v8.7 — Ship UX + gates CLI

- [x] Ship status — `stripe_webhook` from cloud `/billing/config`
- [x] Settings — VPS deploy wizard (`VpsDeployHints.tsx`)
- [x] Release notes fallback when GitHub body empty
- [x] `./scripts/peeknook-gates-status.sh` — CLI gate summary

## v8.8 — v0.2.7 ship ✅

- [x] Bump **0.2.7** — VPS wizard + stripe webhook status + gates CLI
- [x] `./scripts/peeknook-ship-release.sh 0.2.7`
- [x] Verify release after CI
