...
def parse_source_form_data(
    ...
    # Create SourceCreate instance — never pass both notebook_id and notebooks
    try:
        source_data = SourceCreate(
            type=type,
            notebook_id=notebook_id if not notebooks_list else None,
    ...
    try:
        ...
        if source_data.async_processing:
            ...
            try:
                ...
                from open_notebook.peeknook.hooks import track_source_created

                track_source_created(
                    str(source.id),
                    source_data.notebooks or [],
                    source_data.title,
                    source_data.type,
                    file_path or source_data.file_path,
                )
