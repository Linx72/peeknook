...
async def _stream_chat_execute(request: ExecuteChatRequest) -> AsyncGenerator[str, None]:
    try:
        full_session_id = (
            request.session_id
            if request.session_id.startswith("chat_session:")
        ...
        for i in range(0, len(ai_text), chunk_size):
            part = ai_text[i : i + chunk_size]
            yield f"data: {json.dumps({'type': 'ai_chunk', 'content': part})}\n\n"

        await session.save()
        yield f"data: {json.dumps({'type': 'complete', 'content': ai_text})}\n\n"
    except Exception as e:
...
async def execute_chat_stream(request: ExecuteChatRequest):
    """Stream notebook chat response (SSE chunks)."""
    return StreamingResponse(
        _stream_chat_execute(request),
