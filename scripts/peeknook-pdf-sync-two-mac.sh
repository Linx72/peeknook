...
if [[ "$ROLE" == "push" ]]; then
  ...
  HANDOFF="$HANDOFF" SRC_ID="$SRC_ID" NB_ID="$NB_ID" CLOUD="$CLOUD" python3 <<'PY'
import json, os, pathlib
path = pathlib.Path(os.environ["HANDOFF"])
path.write_text(json.dumps({
  "source_id": os.environ["SRC_ID"],
  "notebook_id": os.environ["NB_ID"],
  "cloud_url": os.environ["CLOUD"],
  "cloud_email": os.environ.get("PEEKNOOK_CLOUD_EMAIL"),
  "machine_b": {
    "pull": "./scripts/peeknook-pdf-sync-two-mac.sh pull",
    "record": "./scripts/peeknook-two-mac-physical-record.sh",
  },
}, indent=2))
